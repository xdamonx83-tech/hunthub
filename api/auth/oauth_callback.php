...
if (!function_exists('app_origin')) {
  function app_origin(): string {
    $proto = !empty($_SERVER['HTTP_X_FORWARDED_PROTO'])
      ...
      : (((($_SERVER['HTTPS'] ?? '') !== 'off') || (isset($_SERVER['SERVER_PORT']) && (int)$_SERVER['SERVER_PORT'] === 443)) ? 'https' : 'http');
...
if (!$resp || !preg_match('/is_valid\s*:\s*true/i', (string)$resp)) {
  throw new RuntimeException('Steam OpenID not valid' . ($cerr ? " ($cerr)" : ''));
    ...
    $avatar  = null;
    $display = 'Steam User '.$steamId;
    $apiKey  = $config['steam']['webapi_key'] ?? null;
    if ($apiKey && $apiKey !== 'STEAM_WEBAPI_KEY_HERE') {
      $url     = ($config['steam']['playersummaries_url'] ?? 'https://api.steampowered.com/ISteamUser/GetPlayerSummaries/v2/')
               . '?' . http_build_query(['key' => $apiKey, 'steamids' => $steamId]);
  ...
  switch ($provider) {
    case 'google':
      ...
      if (!$sub) {
        throw new RuntimeException('No Google sub');
      }
    ...
    case 'facebook':
      ...
      $token = http_get_json(($p['token_url'] ?? 'https://graph.facebook.com/v19.0/oauth/access_token') . '?' . http_build_query([
        'client_id'     => $p['client_id'],
        'client_secret' => $p['client_secret'],
        'code'          => $code,
        'redirect_uri'  => $redir,
      ...
      $userinfo = http_get_json(($p['userinfo_url'] ?? 'https://graph.facebook.com/me?fields=id,name,email,picture') . '&' . http_build_query([
      ...
      if (!$id) {
        throw new RuntimeException('No Facebook id');
      }
    ...
    case 'twitch':
      ...
      if (!$id) {
        throw new RuntimeException('No Twitch id');
      }
  ...
  throw new RuntimeException('Provider not handled');
